...
if __name__ == '__main__':
    ...
    """Parameters"""
    expt = '01deg_jra55v13_ryf_AKM_IPOCtrl'
    frequency = '1 monthly'
    year_all = range(2145, 2156) #range(2144, 2156)
#    path_output = '/g/data/e14/mv7494/IPOAnt_analysis/data_AKM_IPOCtrl_AABW_final/'
    path_output = '/g/data/e14/mv7494/IPOAnt_analysis/data_AKM_OSM_IPOCtrl_AABW_final/'
    year = int(sys.argv[1])
    ...
    """Load data"""
    session = cc.database.create_session('local_cc_test.db')
...
# hmaurice: for Adele's runs, I need to use the net_surface_heating components
#        net_sfc_heating = cc.querying.getvar(expt, 'net_sfc_heating',  session, frequency=frequency,chunks={'time': '200MB'})
        swflx     = cc.querying.getvar(expt, 'swflx', session, frequency=frequency, chunks={'time': '200MB'})
        lw_heat   = cc.querying.getvar(expt, 'lw_heat', session, frequency=frequency, chunks={'time': '200MB'})
        sens_heat = cc.querying.getvar(expt, 'sens_heat', session, frequency=frequency, chunks={'time': '200MB'})
        evap_heat = cc.querying.getvar(expt, 'evap_heat', session, frequency=frequency, chunks={'time': '200MB'})
...
# hmaurice: for Adele's run
        swflx = swflx.sel(time=time_slice, yt_ocean=lat_slice)
        lw_heat = lw_heat.sel(time=time_slice, yt_ocean=lat_slice)
        sens_heat = sens_heat.sel(time=time_slice, yt_ocean=lat_slice)
        evap_heat = evap_heat.sel(time=time_slice, yt_ocean=lat_slice)
...
#        net_surface_heating = net_sfc_heating + frazil_3d_int_z
        net_surface_heating = swflx + lw_heat + sens_heat + evap_heat + frazil_3d_int_z
        # W/m2
    ...
    ds.to_netcdf(
        ...
        encoding=enc)
    mask_DSW.to_netcdf(
        path_output + 'mask_DSW' + 
        expt + '_' + frequency[0:3:2] + '_' + str(year) + '.nc',
        encoding=enc)
#    area_t_DSW.to_netcdf(
#        path_output + 'area_t_DSW' + 
#        expt + '_' + frequency[0:3:2] + '_' + str(year) + '.nc',
#        encoding=enc)
